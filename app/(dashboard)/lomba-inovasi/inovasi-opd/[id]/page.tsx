...
export default function CompetitionDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [tab, setTab] = useState<(typeof detailTabs)[number]>("Informasi Inovasi");
  ...
  const isAdmin = user?.role.toLocaleLowerCase("id-ID").includes("admin");

  return (
    <AppSidebarLayout><main className="min-h-screen bg-[#f4f6f9] p-5 lg:p-8"><AppPageHeader title="Detail Inovasi Lomba" description="Lomba Inovasi Perangkat Daerah / Inovasi OPD / Detail" />
      ...
      {tab === "Verifikasi" ? <DetailSection title="Verifikasi Peserta"><div className="grid gap-6 lg:grid-cols-2"><div className="space-y-2">{verificationItems.map((item) => <label key={item} className="flex gap-2 text-sm"><input type="checkbox" checked={Boolean(checks[item])} onChange={(event) => setChecks((current) => ({ ...current, [item]: event.target.checked }))} />{item}</label>)}</div><div><textarea value={notes} onChange={(event) => setNotes(event.target.value)} placeholder="Catatan Verifikasi" className="min-h-28 w-full rounded-lg border p-3 text-sm" />{isAdmin ? <div className="mt-3 flex flex-wrap gap-2"><Button onClick={() => updateStatus({ action: "verify", decision: "VERIFIED", checklist: checks, notes })}>Lolos Verifikasi</Button><Button variant="outline" onClick={() => updateStatus({ action: "verify", decision: "NEEDS_REVISION", checklist: checks, notes })}>Perlu Perbaikan</Button><Button variant="destructive" onClick={() => updateStatus({ action: "verify", decision: "REJECTED", checklist: checks, notes })}>Tidak Lolos</Button></div> : null}</div></div></DetailSection> : null}
